...
[
eff_{it} = \beta_1 \ln(pgdp_{it}) + \beta_2 \left[\ln(pgdp_{it})\right]^2 + \gamma X_{it} + \mu_i + \lambda_t + \varepsilon_{it}
]

其中：

* ( eff_{it} )：碳排放效率
* ( \ln(pgdp_{it}) )：人均GDP对数（经中心化处理）
* ( X_{it} )：控制变量向量
* ( \mu_i )：个体固定效应（城市）
* ( \lambda_t )：时间固定效应（年份）
...
本项目用于学术研究与方法复现，相关结果仅供参考。
如用于进一步研究，请结合具体数据背景进行解释。
